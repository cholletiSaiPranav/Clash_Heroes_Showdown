...
//   <div class="wrap">
//     <div class="bar">
//       <div class="title">🎮 Clash_Heroes_Showdown_Play <span class="pill">v2.0</span></div>
//       <div class="row">
//         <button class="btn ghost" id="backBtn" title="Go Back">⟵ Back</button>
//         <button class="btn ghost" id="homeBtn" title="Home">🏠 Home</button>
//       </div>
//     </div>

//     <!-- HOME -->
//     <section id="screen-home" class="menu active">
//            <p class="subtitle">A Multiplayer-Game created by : <span class="hl">CHSP</span>.<br> Currently this game can be played in single PC <br>Flow: <strong>Try Game → Main Menu → Start Game → Modes → VS Mode → Character Select → Play      
//         <br>    (!STILL UNDER DEVELOPMENT!)<br></strong>contact me: <span class="hl">cholletisaipranav@gmail.com</span></p>
//       <div class="grid cols-2">
//         <div class="card">
//           <div class="title">Home</div>
//           <div class="grid cols-2">
//             <button class="btn primary" data-nav="main">▶ Try Game</button>
//             <button class="btn" data-nav="keys">⌨ Key Settings</button>
//             <button class="btn" disabled>🔐 Login (stub)</button>
//             <button class="btn" disabled>📝 Register (stub)</button>
//           </div>
//         </div>
//         <div class="card">
...
//           <div class="grid">
//             <button class="btn primary" data-nav="modes">▶ Start Game</button>
//             <button class="btn" data-nav="characters">👤 Character List</button>
//             <button class="btn" data-nav="keys">⌨ Key Settings</button>
//             <button class="btn" data-nav="guide">📘 Guide</button>
//             <button class="btn" data-nav="network">🌐 Network Game</button>
//           </div>
//         </div>
//         <div class="card">
...
//       <div class="title">Network Game</div>
//       <div class="grid cols-2">
//         <div class="card">
...
//           <div class="grid cols-2">
//             <button class="btn primary" data-nav="vs-select">⚔ VS Mode</button>
//             <button class="btn" disabled>🌪 Survival (todo)</button>
//           </div>
//         </div>
//         <div class="card">
...
//       <div class="title">VS Mode — Character Select</div>
//       <div class="two-pane">
...
//             <button class="btn" data-nav="modes">⟵ Back to Modes</button>
//           </div>
//         </div>
...
//               <button class="btn" id="resetKeys" type="button">↺ Reset</button>
//             </div>
//           </form>
//         </div>
//         <div class="card">
...
// <ul>
//   <li>
//     <strong style="color:#e67e22;">Swordsman</strong> 🗡️ (HP: 120, Color: <span style="color:#e67e22;">#e67e22</span>)<br>
//     Basic Attack: A powerful melee slash that deals <strong>25 damage</strong>.<br>
//     Special Ability: Temporarily gains speed<strong2× movement speed</strong> for <strong>3 seconds</strong>, allowing quick attacks or escapes.
//   </li>

//   <li>
//     <strong style="color:#2ecc71;">Archer </strong> 🏹 (HP: 100, Color: <span style="color:#2ecc71;">#2ecc71</span>)<br>
...
//   <li>
//     <strong style="color:#3498db;">Icezzy</strong> ❄️ (HP: 105, Color: <span style="color:#3498db;">#3498db</span>)<br>
...
//   <li>
//     <strong style="color:#2ecc40;">Poisonerer</strong> ☠️ (HP: 95, Color: <span style="color:#2ecc40;">#2ecc40</span>)<br>
...
//   <li>
//     <strong style="color:#9b59b6;">Healer</strong> ✨ (HP: 100, Color: <span style="color:#9b59b6;">#9b59b6</span>)<br>
//     Basic Attack: Fires a bolt dealing <strong>10 damage</strong>.<br>
//     Special Ability: Heals for <strong>40 HP over 20 seconds</strong>. Has a cooldown of <strong>90 seconds</strong>.
//   </li>

//   <li>
//     <strong style="color:#1abc9c;">NightGhost</strong> 🌙 (HP: 80, Color: <span style="color:#1abc9c;">#1abc9c</span>)<br>
...
//           <button class="btn warn" data-nav="vs-select">⟵ Exit to Select</button>
//         </div>
...
//         if(isOnline) document.getElementById('playBtn').style.display = 'none'; 
//         else document.getElementById('playBtn').style.display = 'block'; 
//       }
//     }
...
//      * Network Setup    *
//      ********************/
//     const socket = io('https://clash-heroes-showdown.onrender.com'); // Replace with your Render URL
//     let isOnline = false;
//     let playerNum = 0;
...
//         roomIdDisplay.textContent = `Room ID: ${roomId}. Waiting for player...`;
//         isOnline = true;
//         playerNum = 1;
//         show('vsSelect');
//       });
...
//       selected[`p${data.player}`] = data.charId;
//       updateSelections();
//     });

//     socket.on('gameStart', (data) => {
//       selected.p1 = data.p1Char;
//       selected.p2 = data.p2Char;
//       startOnlineMatch(data.playerNum);
//     });

//     socket.on('remoteInput', (data) => {
//       if (data.type === 'hold') {
//         remotePlayer.actions[data.action] = data.value;
//       } else if (data.type === 'trigger') {
//         if (data.action === 'attack') remotePlayer.attack(false);
//         if (data.action === 'special') remotePlayer.useSpecial(localPlayer);
//       }
//     });
...
//      ********************/
//     const CHARACTERS = [
//       { id:'Swordsman', name:'Swordsman', color:'#e67e22', hp:120, desc:'Melee slash 25 dmg. Special: 2× speed (3s).', role:'Swordsman' },
...
//         const canSelect = !isOnline || (side === 'p1' && playerNum === 1) || (side === 'p2' && playerNum === 2);
//         if (canSelect) {
//           card.onclick=()=>{ 
//             selected[side]=ch.id; 
//             updateSelections(); 
//             if (isOnline) socket.emit('choose', ch.id); 
//           }; 
//         }
...
//       if(p1Label) p1Label.textContent = selected.p1 ? CHARACTERS.find(c=>c.id===selected.p1).name : (isOnline && playerNum !== 1 ? 'Waiting...' : 'None'); 
//       if(p2Label) p2Label.textContent = selected.p2 ? CHARACTERS.find(c=>c.id===selected.p2).name : (isOnline && playerNum !== 2 ? 'Waiting...' : 'None'); 
...
//       }
//       facing(){ return this.direction===1 ? '→' : '←'; }
...
//           ctx.fillStyle='#000'; ctx.font='12px Arial'; const y=this.y-6; if(this.isReloading()){ ctx.fillText('Reloading…', this.x-6, y); } else { ctx.fillText('Ammo: '+this.ammo, this.x-6, y); }
//         }
...
//               socket.emit('input', {type: 'hold', action, value: true});
//             } else if (triggerActions.includes(action)) {
//               const flag = action + 'Flag';
//               if (!localPlayer[flag]) {
//                 if (action === 'attack') localPlayer.attack(false);
//                 else localPlayer.useSpecial(remotePlayer);
//                 socket.emit('input', {type: 'trigger', action});
...
//               socket.emit('input', {type: 'hold', action, value: false});
//             } else if (triggerActions.includes(action)) {
//               const flag = action + 'Flag';
...
//       player1 = new Player(120, c1, k.p1);
//       player2 = new Player(canvas.width-160, c2, k.p2);
...
//     function startOnlineMatch(pNum) {
//       playerNum = pNum;
//       isOnline = true;
//       cleanupMatch();
//       const k = loadKeys();
//       const dummyControls = {left:'', right:'', jump:'', down:'', attack:'', defence:'', special:''};
//       const c1 = CHARACTERS.find(c => c.id === selected.p1);
//       const c2 = CHARACTERS.find(c => c.id === selected.p2);
//       if (playerNum === 1) {
//         player1 = new Player(120, c1, k.p1);
//         player2 = new Player(canvas.width - 160, c2, dummyControls);
//         localPlayer = player1;
//         remotePlayer = player2;
//       } else {
//         player1 = new Player(120, c1, dummyControls);
//         player2 = new Player(canvas.width - 160, c2, k.p1); // Use p1 keys for local player
//         localPlayer = player2;
...
//     function handlePlayerControls(p, opponent) {
...
//       // Set actions for local mode
//       if (!isOnline) {
//         player1.actions.left = keysDown[player1.controls.left];
//         player1.actions.right = keysDown[player1.controls.right];
//         player1.actions.jump = keysDown[player1.controls.jump];
//         player1.actions.down = keysDown[player1.controls.down];
//         player1.actions.defence = keysDown[player1.controls.defence];

//         player2.actions.left = keysDown[player2.controls.left];
//         player2.actions.right = keysDown[player2.controls.right];
//         player2.actions.jump = keysDown[player2.controls.jump];
//         player2.actions.down = keysDown[player2.controls.down];
//         player2.actions.defence = keysDown[player2.controls.defence];

//         // Triggers for local
//         const kc1 = player1.controls;
//         if (keysDown[kc1.attack]) {
//           if (!keysDown['_atk_p1']) {
//             player1.attack(false);
//             keysDown['_atk_p1'] = true;
//           }
//         } else keysDown['_atk_p1'] = false;
//         if (keysDown[kc1.special]) {
//           if (!keysDown['_sp_p1']) {
//             player1.useSpecial(player2);
//             keysDown['_sp_p1'] = true;
//           }
//         } else keysDown['_sp_p1'] = false;

//         const kc2 = player2.controls;
//         if (keysDown[kc2.attack]) {
//           if (!keysDown['_atk_p2']) {
//             player2.attack(false);
//             keysDown['_atk_p2'] = true;
//           }
//         } else keysDown['_atk_p2'] = false;
//         if (keysDown[kc2.special]) {
//           if (!keysDown['_sp_p2']) {
//             player2.useSpecial(player1);
//             keysDown['_sp_p2'] = true;
//           }
//         } else keysDown['_sp_p2'] = false;
//       }
//       // Controls (hold actions)
//       handlePlayerControls(player1, player2);
//       handlePlayerControls(player2, player1);
//       // Updates
...
const io = new Server(server, {
  cors: {
    origin: 'https://clash-heroes-showdown.onrender.com', // Restrict to your Render URL
    methods: ['GET', 'POST']
  }
});
...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', (callback) => {
    const roomId = Math.random().toString(36).substring(7);
    rooms[roomId] = { host: socket.id, players: [socket.id], playerNums: { [socket.id]: 1 }, choices: Array(MAX_PLAYERS + 1).fill(null) };
    socket.join(roomId);
    callback(roomId);
  });
  ...
  socket.on('joinRoom', (roomId, callback) => {
    if (rooms[roomId] && rooms[roomId].players.length < MAX_PLAYERS) {
      const assignedNum = rooms[roomId].players.length + 1;
      rooms[roomId].players.push(socket.id);
      rooms[roomId].playerNums[socket.id] = assignedNum;
      socket.join(roomId);
      io.to(roomId).emit('playerJoined', { playerId: assignedNum });
      callback(true, assignedNum);
    } else {
      callback(false);
    }
  });
  ...
  socket.on('choose', (data) => {
    const roomId = Array.from(socket.rooms).find(r => r !== socket.id);
    if (!roomId || !rooms[roomId]) return;
    const player = rooms[roomId].playerNums[socket.id];
    rooms[roomId].choices[player] = data.charId;
    io.to(roomId).emit('playerChose', { player, charId: data.charId });
  });

  socket.on('startGame', () => {
    const roomId = Array.from(socket.rooms).find(r => r !== socket.id);
    if (!roomId || socket.id !== rooms[roomId].host) return; // Only host can start
    io.in(roomId).fetchSockets().then((sockets) => {
      sockets.forEach((s) => {
        const pn = rooms[roomId].playerNums[s.id];
        s.emit('gameStart', {
          playerNum: pn,
  ...
  socket.on('input', (data) => {
    const roomId = Array.from(socket.rooms).find(r => r !== socket.id);
    if (roomId) {
      data.playerId = rooms[roomId].playerNums[socket.id];
      socket.to(roomId).emit('remoteInput', data);
    }
  });
  ...
  socket.on('disconnect', () => {
    ...
    if (roomId && rooms[roomId]) {
      const disconnectedPlayerId = rooms[roomId].playerNums[socket.id];
      rooms[roomId].players = rooms[roomId].players.filter(id => id !== socket.id);
      delete rooms[roomId].playerNums[socket.id];
      rooms[roomId].choices[disconnectedPlayerId] = null;
